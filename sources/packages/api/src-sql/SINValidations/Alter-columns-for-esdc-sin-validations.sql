...
ALTER TABLE
  sims.sin_validations DROP COLUMN given_name_sent,
...
ALTER TABLE
  sims.sin_validations
ADD
  COLUMN IF NOT EXISTS manual_edited_by INT REFERENCES sims.users (id) ON DELETE
SET
  NULL,
ADD
  COLUMN IF NOT EXISTS manual_edited_date TIMESTAMP WITH TIME ZONE,
ADD
  COLUMN IF NOT EXISTS manual_edited_note_id INT REFERENCES sims.notes (id) ON DELETE
SET
  NULL;

-- ## Comments
COMMENT ON COLUMN sims.sin_validations.sin_status IS 'Overall SIN validation status (e.g. 1-Passed, 2-Under Review).';
...
COMMENT ON COLUMN sims.sin_validations.manual_edited_by IS 'User that manually edited the record.';

COMMENT ON COLUMN sims.sin_validations.manual_edited_date IS 'Date and time that a user manually edited the record.';

COMMENT ON COLUMN sims.sin_validations.manual_edited_note_id IS 'Note that explains why the record was manually edited.';
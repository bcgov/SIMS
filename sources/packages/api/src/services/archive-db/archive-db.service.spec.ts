...
const createIndividuals = async (service: ArchiveDbService) => {
  for (const item of testData) {
    ...
    await service.connection.query("INSERT INTO individual(sin,permanent_disability_flg,date_of_birth) VALUES ($1,$2,$3)", [
      sin,
      item.pd,
      item.dob,
    ]);
...
const removeIndividuals = async (service: ArchiveDbService) => {
  for (const item of testData) {
    await service.connection.query(`DELETE FROM individual where sin=${item.sin}`);
  }
} 

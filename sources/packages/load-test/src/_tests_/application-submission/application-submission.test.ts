...
export function setup(): SetupData {
  const setupEndpoint = `application-submission/setup/${ITERATIONS}`;
  const credentials = getLoadTestGatewayCredentials();
  const response = loadTestPostCall(setupEndpoint, credentials);
  const applicationIds = response.json() as number[];
  return { applicationIds, credentials };
}
...
export default function (setupData: SetupData) {
  const applicationId =
    setupData.applicationIds[execution.scenario.iterationInTest];
  const submitEndpoint = `application-submission/submit/${applicationId}`;
  const submitResponse = loadTestPostCall(
    submitEndpoint,
    setupData.credentials
  );
  check(submitResponse, {
    "Submitted with success": (r) => r.status === 201,
  });
  sleep(1);
}

...
from
    ...
    inner join sims.ecert_feedback_errors on ecert_feedback_errors.id = disbursement_feedback_errors.ecert_feedback_error_id
    inner join sims.disbursement_schedules on disbursement_schedules.id = disbursement_feedback_errors.disbursement_schedule_id
    inner join sims.student_assessments on student_assessments.id = disbursement_schedules.student_assessment_id
    inner join sims.education_programs_offerings on education_programs_offerings.id = student_assessments.offering_id
    inner join sims.applications on applications.id = student_assessments.application_id
    inner join sims.students on students.id = applications.student_id
    inner join sims.users on users.id = students.user_id
where

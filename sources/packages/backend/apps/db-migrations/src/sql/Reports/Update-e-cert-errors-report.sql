...
SET
    report_sql = (
        'select
            disbursement_schedules.document_number as "eCert Number",
            applications.application_number as "Application Number",
            users.first_name as "First Name",
            users.last_name as "Last Name",
            CAST(students.birth_date AS varchar) as "Date of Birth",
            ...
            to_char(
                ...
                ''YYYY-MM-DD''
            ) as "Error Logged Date",
            STRING_AGG(ecert_feedback_errors.error_code, '', '') as "Error Codes"
        from
            sims.disbursement_feedback_errors disbursement_feedback_errors
            inner join sims.ecert_feedback_errors ecert_feedback_errors on ecert_feedback_errors.id = disbursement_feedback_errors.ecert_feedback_error_id
            inner join sims.disbursement_schedules disbursement_schedules on disbursement_schedules.id = disbursement_feedback_errors.disbursement_schedule_id
            inner join sims.student_assessments student_assessments on student_assessments.id = disbursement_schedules.student_assessment_id
            ...
            inner join sims.applications applications on applications.id = student_assessments.application_id
        ...
        where
            ...
            and :endDate
        group by
            disbursement_schedules.document_number,
...
WHERE
    report_name = 'ECert_Errors_Report';
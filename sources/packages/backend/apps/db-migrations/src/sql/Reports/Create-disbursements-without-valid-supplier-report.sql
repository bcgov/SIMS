...
VALUES
  ...
  disbursement_sums AS (
    SELECT
      ...
      SUM(
        CASE
          WHEN dv.value_code = '' BCAG '' THEN dv.effective_amount
          ELSE 0
        END
      ) AS bcag_effective_amount,
      SUM(
        CASE
          WHEN dv.value_code = '' SBSD '' THEN dv.effective_amount
          ELSE 0
        END
      ) AS sbsd_effective_amount,
      SUM(
        CASE
          WHEN dv.value_code = '' BGPD '' THEN dv.effective_amount
          ELSE 0
        END
      ) AS bgpd_effective_amount
    FROM
      sims.students AS students
      JOIN sims.applications AS applications ON applications.student_id = students.id
      JOIN sims.student_assessments AS student_assessments ON applications.current_assessment_id = student_assessments.id
      JOIN sims.disbursement_schedules AS disbursement_schedules ON student_assessments.id = disbursement_schedules.student_assessment_id
      AND disbursement_schedules.disbursement_schedule_status = '' Sent ''
      AND disbursement_schedules.date_sent between :startDate
      AND :endDate
      JOIN sims.cas_suppliers AS cas_suppliers ON students.id = cas_suppliers.student_id
      ...
      LEFT JOIN sims.disbursement_values AS dv ON dv.disbursement_schedule_id = disbursement_schedules.id
    WHERE
...
FROM
  disbursement_sums
  JOIN sims.students AS students ON students.id = disbursement_sums.student_id
  JOIN sims.users AS users ON students.user_id = users.id
  JOIN sims.sin_validations AS sin_validations ON students.sin_validation_id = sin_validations.id
ORDER BY
  students.id;

...
describe(describeProcessorRootTest(QueueNames.IER12Integration), () => {
  ...
  it("Should generate IER12 records for all disbursements from the same application when only one disbursement was updated in range.", async () => {
    // Arrange
    const defaultIERFileGenerationStartDate = new Date(
      dateUtils.getISODateOnlyString(dateUtils.addDays(-1)),
    );
    ...
    expect(line2.length).toBe(IER_RECORD_EXPECTED_LENGTH);
    expect(line2).toBe(
      `${assessmentId}${secondDisbursementId}${defaultApplicationNumber}A1B2C3D4    242963189Doe                      John           19980113B   SI  NONENAddress Line 1           Address Line 2           Victoria                 BC  Z1Z1Z1          Program name                                                               undergraduateCertificate 0001    8   0512123401234ADR1                         6${currentOfferingId}${parentOfferingId}                              2000081620001205000033330000004444000000555500000066660050100F2000060120002001COMP20000601000010000000015161000008040500NNNNN            20000801                Required  Pending   20001011                        CSLF0000000000BCSL0001516100CSGP0000123400CSGD0000597800CSGF0000910100CSGT0001213100BCAG0000181900SBSD0000002200BGPD0000202100    0000000000`,
    );
  });

  it("Should generate IER12 records for all disbursements from the same application when only one disbursement feedback error was updated in range.", async () => {
    // Arrange
    const inRangeFeedbackErrorDate = dateUtils.addHours(
      1,
    ...
    // Ensure user update date is outside the generation range.
    await db.user.update(application.student.user.id, {
      updatedAt: dateOutsideDefaultIERFileGenerationRange,
    });
    ...
    const feedbackError = createFakeDisbursementFeedbackError(
      { disbursementSchedule: firstDisbursement, eCertFeedbackError },
      {
        initialValues: {
          // Only feedback error update is inside the selected generation range.
          updatedAt: inRangeFeedbackErrorDate,
        },
      },
    );
    ...
    // Queued job.
    // Use explicit generated date to keep assertions deterministic.
    ...
    expect(line1.length).toBe(IER_RECORD_EXPECTED_LENGTH);
    expect(line1).toBe(
      `${assessmentId}${firstDisbursementId}${defaultApplicationNumber}A1B2C3D4    242963189Doe                      John           19980113B   SI  NONENAddress Line 1           Address Line 2           Victoria                 BC  Z1Z1Z1          Program name                                                               undergraduateCertificate 0001    8   0512123401234ADR1                         6${currentOfferingId}${parentOfferingId}                              2000081620001205000033330000004444000000555500000066660050100F2000060120002001COMP20000601000010000000015161000008040500NNNNN            20000801         Sent      20000816                        CSLF0000100000BCSL0000000000CSGP0000200000CSGD0000300000CSGF0000400000CSGT0000500000BCAG0000700000SBSD0000900000BGPD0000800000    0000000000`,
    );
  ...
  it("Should generate an IER12 file with one record and with interface policy calculated values for a married student with dependents when the interface policy applies to the application.", async () => {
    ...
    const workflowCalculatedDataWithInterfacePolicy = {
      ...
      interfaceNeed: 7031,
      interfaceChildCareCosts: 1000,
      interfaceEducationCosts: 5418,
      interfaceTransportationAmount: 375,
      interfaceAdditionalTransportationAmount: 238,
    };
    const workflowData: IER12WorkflowData = {
      ...WORKFLOW_DATA_MARRIED_WITH_DEPENDENTS,
      calculatedData: workflowCalculatedDataWithInterfacePolicy,
    };
    const testInputData = {
      student: JANE_MONONYMOUS_FROM_OTHER_COUNTRY,
      application: {

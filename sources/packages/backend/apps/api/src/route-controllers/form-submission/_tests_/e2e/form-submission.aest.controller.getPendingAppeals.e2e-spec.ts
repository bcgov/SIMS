...
describe("FormSubmissionAESTController(e2e)-getPendingAppeals", () => {
  ...
  it("Should return only appeals linked to an application when the type filter is application.", async () => {
    // Arrange
    // Add a unique identifier in the student lastName to isolate the data.
    const uniqueIdentifier = faker.string.uuid();
    const userWithApplication = createFakeUser();
    ...
    const endpoint = `/aest/form-submission/pending-appeals?page=0&pageLimit=100&sortField=submittedDate&sortOrder=DESC&searchCriteria=${uniqueIdentifier}&applicationFilter=withApplication`;
    const token = await getAESTToken(AESTGroups.BusinessAdministrators);
  ...
  it("Should return only appeals with type other when the type filter is other.", async () => {
    // Arrange
    // Add a unique identifier in the student lastName to isolate the data.
    const uniqueIdentifier = faker.string.uuid();
    const userWithApplication = createFakeUser();
    ...
    const endpoint = `/aest/form-submission/pending-appeals?page=0&pageLimit=100&sortField=submittedDate&sortOrder=DESC&searchCriteria=${uniqueIdentifier}&applicationFilter=withoutApplication`;
    const token = await getAESTToken(AESTGroups.BusinessAdministrators);

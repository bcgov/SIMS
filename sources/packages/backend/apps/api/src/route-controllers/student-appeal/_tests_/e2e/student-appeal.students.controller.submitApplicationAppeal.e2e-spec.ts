...
describe("StudentAppealStudentsController(e2e)-submitApplicationAppeal", () => {
  ...
  it(
    "Should throw unprocessable entity exception when student submit a step parent waiver appeal" +
      " for an application which has only one reported parent.",
    async () => {
      // Arrange
      // Create student to submit application.
      const student = await saveFakeStudent(appDataSource);
      ...
      // Create only one supporting user parent for the application.
      await db.supportingUser.save(
        createFakeSupportingUser(
          { application },
          {
            initialValues: {

/*
 * Update CSLF overawards from SFAS integration to disbursement overawards table.
 */
UPDATE
	sims.disbursement_overawards disbursement_overawards
SET
	overaward_value = sfas_individuals.csl_overaward,
	updated_at = NOW(),
	modifier = users.id
FROM
...
WHERE
	...
	AND disbursement_overawards.overaward_value <> sfas_individuals.csl_overaward;

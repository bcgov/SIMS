...
describe(`E2E Test Workflow part-time-assessment-${PROGRAM_YEAR}-costs-scholarship-bursary.`, () => {
  ...
  it(
    ...
    async () => {
      ...
      assessmentConsolidatedData.programYearTotalScholarshipsBursaries = 0;
      // Act
      const calculatedAssessment =
        await executeFullTimeAssessmentForProgramYear(
          PROGRAM_YEAR,
  ...
  it(
    "Should calculate $100 for total scholarship and bursaries when " +
      "student declares that they received scholarship and bursaries " +
    ...
    async () => {
      ...
      assessmentConsolidatedData.programYearTotalScholarshipsBursaries = 1000;
      // Act
      const calculatedAssessment =
        await executeFullTimeAssessmentForProgramYear(
          PROGRAM_YEAR,
      ...
      // PY limit is $1800 before any declared scholarship or bursary amounts are considered towards contribution.
      // Student has already had an exemption of $1000 in the current program year.
      expect(
        calculatedAssessment.variables
          .calculatedDataRemainingScholarshipsBursariesLimit,
      ).toBe(800);
      // The amount declared by the student is used in future applications towards the same PY limit.
      // If the remaining limit is greater than the declared amount, this should be the student declared value.
      expect(
        calculatedAssessment.variables
          .calculatedDataExemptScholarshipsBursaries,
      ).toBe(800);
